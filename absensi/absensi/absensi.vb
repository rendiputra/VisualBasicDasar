...
Public Class absensi
    ...
    Sub tampiltextbox()
        CONN.Close()
        Call koneksi()
        CMD = New OdbcCommand("SELECT * FROM tabsensi WHERE NIP='" & txtnip.Text & "'", CONN)
    ...
    Private Sub btnhapus_Click(sender As Object, e As EventArgs) Handles btnhapus.Click
        ...
        Else
            If MessageBox.Show("Apakah Ingin Menghapus Data?", "Konfirmasi Perubahan Data", MessageBoxButtons.YesNo, MessageBoxIcon.Information) = DialogResult.Yes Then
                Call koneksi()
                Dim hapus As String = "delete from tabsensi where NIP='" & txtnip.Text & "'"
    ...
    Private Sub btnedit_Click(sender As Object, e As EventArgs) Handles btnedit.Click
        If txtnip.Text = "" Or txtnama.Text = "" Or txttanggal.Text = "" Or txtizin.Text = "" Or txthadir.Text = "" Or txtsakit.Text = "" Or txtalpa.Text = "" Then
            MsgBox("Silahkan Isi Semua From")
        Else
            Call koneksi()
            Dim edit As String = "update tabsensi set nama='" & txtnama.Text & "',tanggal='" & txttanggal.Text & "',hadir='" & txthadir.Text & "',izin='" & txtizin.Text & "',sakit='" & txtsakit.Text & "',alpa='" & txtalpa.Text & "'where NIP='" & txtnip.Text & "'"
    ...
    Private Sub btntambah_Click(sender As Object, e As EventArgs) Handles btntambah.Click
        ...
        Else
            Call koneksi()
            Dim Simpan As String = "insert into tabsensi values ('" & txtnip.Text & "','" & txtnama.Text & "', '" & txttanggal.Text & "','" & txthadir.Text & "', '" & txtsakit.Text & "', '" & txtizin.Text & "' ,'" & txtalpa.Text & "')"
    ...
    Sub cari()
        Call koneksi()
        DA = New OdbcDataAdapter("Select * from tabsensi Where NIP like '" & Me.txtcari.Text & "%'", CONN)
        DS = New DataSet
        DS.Clear()
        DA.Fill(DS, "tabsensi")
        DataGridView1.DataSource = (DS.Tables("tabsensi"))
        Dim tampil1 As String
        tampil1 = "select * from tabsensi where NIP ='" & txtcari.Text & "'"
